...
(0, cucumber_1.When)('I enter invalid credentials {int} times', async function (attempts) {
...
(0, cucumber_1.Then)('my account should be temporarily locked', async function () {
...
(0, cucumber_1.Then)('I should not be able to login for {int} minutes', async function (minutes) {
    ...
    (0, globals_1.expect)(lockoutExpiry.getTime()).toBeCloseTo(expectedExpiry.getTime(), -4); // Within 10 seconds
});
...
(0, cucumber_1.When)('I remain inactive for {int} minutes', async function (minutes) {
...
(0, cucumber_1.Then)('I should be redirected to the login page', async function () {
    await this.page.waitForURL('/login');
    const currentUrl = this.page.url();
    (0, globals_1.expect)(currentUrl).toContain('/login');
});
(0, cucumber_1.Then)('I should see a session timeout message', async function () {
...
(0, cucumber_1.Then)('I should not be able to access protected pages', async function () {
...
(0, cucumber_1.Then)('I should be logged in on both devices', async function () {
...
(0, cucumber_1.Then)('I should remain logged in on device B', async function () {
    const sessionValid = await this.verifySession(this.deviceBSessions.token);
    (0, globals_1.expect)(sessionValid).toBe(true);
});
(0, cucumber_1.Then)('device A session should be invalidated', async function () {
...
(0, cucumber_1.Then)('only matched users should see my profile', async function () {
...
(0, cucumber_1.Then)('other users should not see when I was last online', async function () {
    const otherUser = await this.createTestUser('other-user');
    const profile = await this.getProfileForUser(this.testUser.id, otherUser.id);
    (0, globals_1.expect)(profile.lastSeen).toBeUndefined();
});
(0, cucumber_1.Then)('my other profile information should remain visible', async function () {
    const otherUser = await this.createTestUser('other-user');
    const profile = await this.getProfileForUser(this.testUser.id, otherUser.id);
...
(0, cucumber_1.Then)('my exact location should not be shared', async function () {
    const user = await this.mockDatabase.users.findById(this.testUser.id);
...
(0, cucumber_1.Then)('location-based matching should be disabled', async function () {
...
(0, cucumber_1.Then)('other users should see my city but not exact location', async function () {
    const otherUser = await this.createTestUser('other-user');
    const profile = await this.getProfileForUser(this.testUser.id, otherUser.id);
    (0, globals_1.expect)(profile.city).toBeTruthy();
    (0, globals_1.expect)(profile.exactLocation).toBeUndefined();
});
(0, cucumber_1.Then)('distance should be shown as approximate ranges', async function () {
    const otherUser = await this.createTestUser('other-user');
    const profile = await this.getProfileForUser(this.testUser.id, otherUser.id);
    (0, globals_1.expect)(profile.distance).toMatch(/^\d+-\d+ km$/);
});
(0, cucumber_1.Then)('my precise coordinates should remain private', async function () {
    const user = await this.mockDatabase.users.findById(this.testUser.id);
...
(0, cucumber_1.Then)('the user should be added to my blocked list', async function () {
...
(0, cucumber_1.Then)('they should not be able to see my profile', async function () {
...
(0, cucumber_1.Then)('they should not be able to message me', async function () {
...
// Reporting
(0, cucumber_1.When)('I select {string} as the reason', async function (reason) {
...
(0, cucumber_1.Then)('the report should be submitted successfully', async function () {
    ...
    const userReport = reports.find(report => report.reporterId === this.testUser.id &&
        report.reportedUserId === this.targetUser.id);
    (0, globals_1.expect)(userReport).toBeTruthy();
});
(0, cucumber_1.Then)('the user should be automatically blocked', async function () {
    const blockedUsers = await this.mockDatabase.blocks.getBlockedUsers(this.testUser.id);
    const isBlocked = blockedUsers.some(user => user.id === this.targetUser.id);
    (0, globals_1.expect)(isBlocked).toBe(true);
});
(0, cucumber_1.Then)('I should receive a confirmation message', async function () {
    ...
    (0, globals_1.expect)(confirmationMessage).toContain('Report submitted successfully');
});
// Message encryption and security
(0, cucumber_1.Given)('I have a conversation with another user', async function () {
...
(0, cucumber_1.Then)('the message should be encrypted in transit', async function () {
    // Verify HTTPS and TLS encryption
    const response = await this.page.waitForResponse(response => response.url().includes('/api/messages') && response.request().method() === 'POST');
    const request = response.request();
    (0, globals_1.expect)(request.url()).toMatch(/^https:/);
});
(0, cucumber_1.Then)('the message should be encrypted at rest', async function () {
...
(0, cucumber_1.Then)('only the recipient and I should be able to read it', async function () {
    // Verify end-to-end encryption keys
    const messageKeys = await this.mockDatabase.messageKeys.getKeys('conversation-123');
    (0, globals_1.expect)(messageKeys.participants).toEqual([this.testUser.id, this.conversationPartner.id]);
});
// Message deletion
(0, cucumber_1.Given)('I have sent messages in a conversation', async function () {
...
(0, cucumber_1.Then)('the message should be removed from my view', async function () {
...
(0, cucumber_1.Then)('the message should be marked as deleted in the database', async function () {
    const messages = await this.mockDatabase.messages.getConversationMessages('conversation-123');
    const deletedMessage = messages.find(msg => msg.senderId === this.testUser.id);
    (0, globals_1.expect)(deletedMessage.deleted).toBe(true);
});
(0, cucumber_1.Then)('the recipient should see {string} placeholder', async function (placeholder) {
...
(0, cucumber_1.Then)('my data export should be prepared within {int} days', async function (days) {
    ...
    (0, globals_1.expect)(new Date(exportRequest.expectedCompletion)).toBeLessThanOrEqual(expectedCompletionDate);
});
(0, cucumber_1.Then)('I should receive a download link when ready', async function () {
    ...
    (0, globals_1.expect)(downloadEmail.body).toContain('https://example.com/export/user-data.zip');
});
(0, cucumber_1.Then)('the export should include all my personal data', async function () {
...
(0, cucumber_1.Then)('my profile should be immediately hidden', async function () {
    const user = await this.mockDatabase.users.findById(this.testUser.id);
    (0, globals_1.expect)(user.profileVisible).toBe(false);
});
(0, cucumber_1.Then)('my data should be permanently deleted within {int} days', async function (days) {
    const user = await this.mockDatabase.users.findById(this.testUser.id);
...
(0, cucumber_1.Then)('I should receive deletion confirmation', async function () {
    const emails = await this.mockEmailService.getEmailsForUser(this.testUser.email);
...
(0, cucumber_1.When)('I make an API request without authentication', async function () {
...
(0, cucumber_1.Then)('the response should contain {string}', async function (expectedText) {
...
(0, cucumber_1.When)('I make {int} API requests within {int} minute', async function (requestCount, minutes) {
    const requests = [];
...
(0, cucumber_1.Then)('subsequent requests should be rate limited', async function () {
...
(0, cucumber_1.Then)('the response should contain rate limit information', async function () {
    const rateLimitedResponse = await this.apiClient.get('/api/user/profile', {
        headers: { Authorization: `Bearer ${this.authToken}` }
    });
    const rateLimitHeaders = rateLimitedResponse.headers;
    (0, globals_1.expect)(rateLimitHeaders['x-ratelimit-limit']).toBeTruthy();
    (0, globals_1.expect)(rateLimitHeaders['x-ratelimit-remaining']).toBeTruthy();
    (0, globals_1.expect)(rateLimitHeaders['x-ratelimit-reset']).toBeTruthy();
});
// Security vulnerability protection
(0, cucumber_1.When)('I send a malicious SQL payload in a request', async function () {
...
(0, cucumber_1.Then)('the request should be processed safely', async function () {
...
// XSS protection
(0, cucumber_1.When)('I submit content with malicious scripts', async function () {
...
(0, cucumber_1.Then)('the content should be safely displayed', async function () {
    const bioContent = await this.page.textContent('[data-testid="user-bio"]');
...
(0, cucumber_1.When)('I enter a weak password {string}', async function (weakPassword) {
...
(0, cucumber_1.Then)('I should see password strength requirements', async function () {
...
(0, cucumber_1.Then)('the password should be rejected', async function () {
    ...
    (0, globals_1.expect)(errorMessage).toContain('Password does not meet requirements');
});
// Password hashing
(0, cucumber_1.Given)('I create an account with password {string}', async function (password) {
    const newUser = {
...
(0, cucumber_1.Then)('my password should be hashed', async function () {
    ...
    (0, globals_1.expect)(this.storedUser.passwordHash).not.toBe(this.secureUser.password);
});
(0, cucumber_1.Then)('the plain text password should not be stored', async function () {
    (0, globals_1.expect)(this.storedUser.password).toBeUndefined();
});
(0, cucumber_1.Then)('the hash should use a secure algorithm', async function () {
    // Check for bcrypt hash format
    (0, globals_1.expect)(this.storedUser.passwordHash).toMatch(/^\$2[aby]\$\d+\$/);
});
...
(0, cucumber_1.Then)('the reset link should expire after {int} hour', async function (hours) {
    ...
    (0, globals_1.expect)(expiryTime.getTime()).toBeCloseTo(expectedExpiry.getTime(), -4);
});
(0, cucumber_1.Then)('the reset token should be single-use', async function () {
    ...
    await this.page.goto(`/reset-password?token=${this.resetToken}`);
    const errorMessage = await this.page.textContent('[data-testid="error-message"]');
    (0, globals_1.expect)(errorMessage).toContain('Invalid or expired reset token');
});
(0, cucumber_1.Then)('the token should be cryptographically secure', async function () {
