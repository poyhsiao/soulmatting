...
Then(
  ...
  async function (this: World, minutes: number) {
...
When(
  ...
  async function (this: World, visibility: string) {
...
When(
  ...
  async function (this: World, reason: string) {
    await this.page.selectOption(
...
When(
  ...
  async function (this: World, messageText: string) {
...
Then(
  ...
  async function (this: World) {
    ...
    expect(messageKeys.participants).toEqual([
      this.testUser.id,
...
Then(
  ...
  async function (this: World, placeholder: string) {
...
Then(
  ...
  async function (this: World, days: number) {
    ...
    expect(new Date(exportRequest.expectedCompletion)).toBeLessThanOrEqual(
...
Then(
  ...
  async function (this: World) {
    ...
    expect(downloadEmail.body).toContain(
      'https://example.com/export/user-data.zip'
    );
  }
);
...
Then(
  ...
  async function (this: World, statusCode: number) {
...
When(
  ...
  async function (this: World, requestCount: number, minutes: number) {
    const requests = [];
...
When(
  ...
  async function (this: World, weakPassword: string) {
...
Then(
  ...
  async function (this: World, hours: number) {
...
// Helper methods
async function createSession(this: World, deviceId: string) {
...
async function invalidateSession(this: World, token: string) {
  await this.authService.invalidateSession(token);
}

async function verifySession(this: World, token: string): Promise<boolean> {
  try {
...
async function createTestUser(this: World, identifier: string) {
  const user = {
...
async function switchToUser(this: World, userId: string) {
  // Switch context to different user for testing
  this.currentUserId = userId;
  const authToken = await getAuthToken.call(this, userId);
  await this.page.evaluate(token => {
    localStorage.setItem('authToken', token);
  }, authToken);
}

async function getAuthToken(this: World, userId: string): Promise<string> {
  return await this.authService.generateToken(userId);
}

function generateTOTP(this: World, secret: string): string {
