...
(0, cucumber_1.When)('I swipe left on a match card', async function () {
  const matchCard = await this.page
...
(0, cucumber_1.Then)(
  ...
  async function () {
    ...
    await (0, test_1.expect)(
      firstMatch.locator('[data-testid="match-age"]')
    ).toBeVisible();
    await (0, test_1.expect)(
...
(0, cucumber_1.Then)(
  ...
  async function () {
    ...
    (0, test_1.expect)(response.status).toBe(200);
    (0, test_1.expect)(response.data.likes).toContainEqual(
...
(0, cucumber_1.Then)(
  ...
  async function () {
    ...
    if (
      response.status === 200 &&
      response.data.notifications.some(n => n.type === 'mutual_match')
    ) {
      (0, test_1.expect)(response.data.notifications).toContainEqual(
        test_1.expect.objectContaining({ type: 'mutual_match' })
      );
    }
  }
);
...
(0, cucumber_1.Then)(
  ...
  async function () {
    ...
    (0, test_1.expect)(
      Array.isArray(this.testData.lastApiResponse.data.matches)
    ).toBe(true);
  }
);
(0, cucumber_1.Then)(
...
(0, cucumber_1.Then)(
  ...
  async function (action) {
    ...
    (0, test_1.expect)(response.status).toBe(200);
    (0, test_1.expect)(response.data[`${action}s`]).toContainEqual(
      test_1.expect.objectContaining({
        targetUserId: test_1.expect.any(String),
      })
    );
  }
);
