...
// Background Steps
Given('I am a registered user with a complete profile', async function (this: CustomWorld) {
...
// Setup Steps
Given('there are other users with compatible profiles', async function (this: CustomWorld) {
...
// Interaction Steps
When('I click the {string} button', async function (this: CustomWorld, action: string) {
...
When('I set age range to {string}', async function (this: CustomWorld, ageRange: string) {
  const [minAge, maxAge] = ageRange.split('-');
  await this.fillField('[data-testid="min-age-input"]', minAge);
  await this.fillField('[data-testid="max-age-input"]', maxAge);
});

When('I set distance to {string}', async function (this: CustomWorld, distance: string) {
  await this.fillField('[data-testid="distance-input"]', distance.replace(' miles', ''));
});

When('I select interests {string}', async function (this: CustomWorld, interests: string) {
...
When('I send a POST request to {string} with the user ID', async function (this: CustomWorld, endpoint: string) {
...
// Assertion Steps
Then('I should see a list of potential matches', async function (this: CustomWorld) {
  await this.waitForElement('[data-testid="match-list"]');
  const matches = await this.page!.locator('[data-testid="match-card"]').count();
  expect(matches).toBeGreaterThan(0);
});

Then('each match should display basic profile information', async function (this: CustomWorld) {
  ...
  await expect(firstMatch.locator('[data-testid="match-photo"]')).toBeVisible();
});

Then('each match should have action buttons \(like, pass, super like\)', async function (this: CustomWorld) {
  const firstMatch = this.page!.locator('[data-testid="match-card"]').first();
  await expect(firstMatch.locator('[data-testid="like-button"]')).toBeVisible();
  await expect(firstMatch.locator('[data-testid="pass-button"]')).toBeVisible();
  await expect(firstMatch.locator('[data-testid="super-like-button"]')).toBeVisible();
});

Then('the match should be recorded in the system', async function (this: CustomWorld) {
...
Then('I should see the next potential match', async function (this: CustomWorld) {
...
Then('the liked user should be notified if it\'s a mutual match', async function (this: CustomWorld) {
  ...
  if (response.status === 200 && response.data.notifications.some((n: any) => n.type === 'mutual_match')) {
...
Then('the response should contain a list of potential matches', async function (this: CustomWorld) {
  expect(this.testData.lastApiResponse.data).toHaveProperty('matches');
  expect(Array.isArray(this.testData.lastApiResponse.data.matches)).toBe(true);
});

Then('each match should include user ID, basic profile info, and compatibility score', async function (this: CustomWorld) {
...
Then('it should register as a {string}', async function (this: CustomWorld, action: string) {
...
Then('the swipe gestures should be smooth and responsive', async function (this: CustomWorld) {
...
// Accessibility Steps
Then('each match card should have proper ARIA labels', async function (this: CustomWorld) {
...
Then('action buttons should be keyboard accessible', async function (this: CustomWorld) {
...
Then('my super like count should decrease by one', async function (this: CustomWorld) {
