...
Given(
  'I have access to the SoulMatting project repository',
...
Given(
  'I have Node.js 22+ and pnpm installed',
  async function (this: CustomWorld) {
    try {
...
Then(
  'each directory should contain appropriate configuration files',
  async function (this: CustomWorld) {
...
Then(
  'the monorepo structure should follow best practices',
  async function (this: CustomWorld) {
    ...
    expect(
      packageJson.workspaces || packageJson.pnpm?.workspaces
    ).toBeDefined();
  }
);
...
When(
  'I examine the Docker configuration files',
  async function (this: CustomWorld) {
    this.dockerConfig = {
    ...
    if (fs.existsSync('services')) {
      const serviceDirs = fs
        ...
        .filter(item => fs.statSync(path.join('services', item)).isDirectory());
...
Then(
  'I should see a "Dockerfile" in each service directory',
  async function (this: CustomWorld) {
...
Then(
  'I should see a "docker-compose.yml" file in the root',
  async function (this: CustomWorld) {
...
Then(
  'I should see a "docker-compose.dev.yml" file for development',
  async function (this: CustomWorld) {
...
Then(
  'each Dockerfile should follow security best practices',
  async function (this: CustomWorld) {
    for (const service of this.dockerConfig.serviceDockerfiles) {
...
Then(
  'the Docker images should be optimized for size and performance',
  async function (this: CustomWorld) {
    for (const service of this.dockerConfig.serviceDockerfiles) {
      if (service.hasDockerfile) {
        const dockerfilePath = path.join(
...
Given(
  'I have the Docker Compose configuration',
  async function (this: CustomWorld) {
...
When(
  'I run "docker-compose -f docker-compose.dev.yml up"',
  async function (this: CustomWorld) {
    try {
...
Then(
  'all required services should start successfully:',
  async function (this: CustomWorld, dataTable) {
      ...
      try {
        switch (service.Service) {
          case 'PostgreSQL':
            ...
            execSync(
              `docker-compose -f docker-compose.dev.yml exec -T postgres pg_isready -h localhost -p ${port}`,
              { stdio: 'pipe' }
            );
            break;
          ...
          case 'Redis':
            ...
            execSync(
              `docker-compose -f docker-compose.dev.yml exec -T redis redis-cli -p ${port} ping`,
              { stdio: 'pipe' }
            );
            break;
        ...
        throw new Error(
          `Service ${service.Service} health check failed: ${error.message}`
        );
      }
    }
  }
...
Then(
  'the development environment should be ready for coding',
  async function (this: CustomWorld) {
    ...
    const serviceList = runningServices
      .trim()
      .split('\n')
      .filter(s => s.length > 0);
...
When(
  'I examine the ".github/workflows" directory',
  async function (this: CustomWorld) {
...
Then(
  'I should see the following workflow files:',
  async function (this: CustomWorld, dataTable) {
...
Then(
  'each workflow should have proper triggers',
  async function (this: CustomWorld) {
    for (const workflow of this.workflows) {
      expect(workflow.content).toMatch(
        /on:\s*[\s\S]*?(push|pull_request|schedule|workflow_dispatch)/i
      );
    }
  }
);
...
Then(
  'the workflows should include automated testing',
  async function (this: CustomWorld) {
    const ciWorkflow = this.workflows.find(w => w.name.includes('ci'));
    expect(ciWorkflow).toBeDefined();
    expect(ciWorkflow.content).toMatch(
      /(npm test|pnpm test|yarn test|jest|vitest)/i
    );
  }
);
...
Then(
  ...
  async function (this: CustomWorld) {
    const securityWorkflow = this.workflows.find(w =>
...
When(
  'I examine the environment configuration',
  async function (this: CustomWorld) {
    this.envFiles = {
...
Then(
  'I should see the following configuration files:',
  async function (this: CustomWorld, dataTable) {
...
Then(
  'each configuration should have appropriate security settings',
  async function (this: CustomWorld) {
...
Then(
  'environment-specific variables should be properly documented',
  async function (this: CustomWorld) {
    if (fs.existsSync('.env.example')) {
      const envExample = fs.readFileSync('.env.example', 'utf8');
